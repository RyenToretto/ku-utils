...
function copyIfNotExists(src, dest) {
  ...
  console.log(`[@ku-utils/types] ✅ 已安装: ${path.relative(projectRoot, dest)}`)
}
...
try {
  const skillsDir = path.join(pkgRoot, 'skills', 'types')
  const targetSkillsDir = path.join(projectRoot, '.cursor', 'skills', 'types')
