...
function copy(src, dest) {
  ...
  console.log(`[@ku-utils/prettier-config] ✅ 已安装: ${path.relative(projectRoot, dest)}`)
}
...
try { copy(path.join(pkgRoot,'skills','prettier-config','SKILL.md'), path.join(projectRoot,'.cursor','skills','prettier-config','SKILL.md')) } catch(e){}
