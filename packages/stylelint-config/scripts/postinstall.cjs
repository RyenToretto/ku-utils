...
function copy(src, dest) {
  ...
  console.log(`[@ku-utils/stylelint-config] ✅ 已安装: ${path.relative(projectRoot, dest)}`)
}
...
try { copy(path.join(pkgRoot,'skills','stylelint-config','SKILL.md'), path.join(projectRoot,'.cursor','skills','stylelint-config','SKILL.md')) } catch(e){}
