...
function copy(src, dest) {
  ...
  console.log(`[@ku-utils/design-tokens] ✅ 已安装: ${path.relative(projectRoot, dest)}`)
}
...
try { copy(path.join(pkgRoot,'skills','design-tokens','SKILL.md'), path.join(projectRoot,'.cursor','skills','design-tokens','SKILL.md')) } catch(e){}
