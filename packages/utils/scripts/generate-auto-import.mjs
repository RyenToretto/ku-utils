 ...
 *
 * 流程：
 *   1. dynamic import dist/index.js
 *   2. Object.keys() 拿到所有命名导出（排除 default 和下划线开头的私有符号）
 *   3. 写回 src/auto-import.ts，下次 build 时被 tsup 重新打包成 dist/auto-import.{js,cjs,d.ts}
 *
 * 调用时机：在 package.json scripts 的 build 中两次 tsup 之间插入本脚本。
 *
 * 设计意图：业务方只要 export 一个新工具函数到 src/index.ts，build 一遍即可让消费方
 * 的 Nuxt auto-import preset 自动获得该符号，无需手动维护 auto-import.ts。
 */
