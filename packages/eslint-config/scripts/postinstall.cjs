...
function copy(src, dest) {
  ...
  console.log(`[@ku-utils/eslint-config] ✅ 已安装: ${path.relative(projectRoot, dest)}`)
}
...
try { copy(path.join(pkgRoot,'skills','eslint-config','SKILL.md'), path.join(projectRoot,'.cursor','skills','eslint-config','SKILL.md')) } catch(e){}
