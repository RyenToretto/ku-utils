...
export class ReportDemoController {
  ...
  private token = 'nuxt-user-1';
  private orderId = 'nuxt-order-1';
  private sequence = 1;
  ...
  async simulatePaymentWithFreshAlive(): Promise<void> {
    this.token = `nuxt-pay-user-${this.sequence}`;
    this.orderId = `nuxt-order-${Date.now()}-${this.sequence++}`;
  ...
  async queueBeforeInit(): Promise<void> {
    this.token = `nuxt-preinit-${this.sequence}`;
    const tracker = this.createTracker(this.token);
  ...
  async switchToken(): Promise<void> {
    this.token = `nuxt-user-${++this.sequence}`;
  ...
  async verifyDedupe(): Promise<void> {
    await this.ensureTracker();
    const orderId = `nuxt-dedupe-${this.sequence++}`;
  ...
  async reportAttribute(): Promise<void> {
    ...
    const body = { level: 'gold', source: 'playground-nuxt4' };
  ...
  private createTracker(token: string): TrackerInstance {
    return createXhReport({
      attrUrl: ATTR_URL,
      isDev: false,
      pkg: 'com.ku-utils.playground-nuxt4.demo',
