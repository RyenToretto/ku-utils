...
> 样板：`src/modules/_example/simpleExample/`（开发态 Demo）。自定义列样板：`src/modules/_example/customColumns/`。
...
域级：`modules/<域>/_router`、`_maps`、`_mock`、`index.vue`（侧栏 + `<router-view>`）。
...
- 筛选：`DoFilterPanel`；placeholder「不限」；单选且选项 ≤3 用 `el-radio-button`；日期在前、radio 在后
