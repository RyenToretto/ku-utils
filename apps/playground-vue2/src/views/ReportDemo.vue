<template>
  <main class="report-demo">
    <header class="hero">
      ...
      <p class="brand">DO POWER / REPORT LAB / VUE 2</p>
      <h2>支付上报与事件池可靠性验证</h2>
      <p>
        使用
        <code>createXhReport</code>
        自动初始化并请求真实上报端点，验证 SDK 在 Vue 2.7 中的 alive 屏障、FIFO
        分批、双通道、失败恢复与身份隔离。测试使用假 pkg。
      </p>
      <div class="identity">
...
export default {
  ...
  created() {
    this.controller = new ReportDemoController((snapshot) => {
      this.snapshot = snapshot;
    });
  },
  mounted() {
    this.run(this.controller.initialize);
  },
  methods: {
    runBurst() {
      return this.run(() => this.controller.burst(40));
    },
    async run(action) {
      this.busy = true;
      try {
        await action.call(this.controller);
      } finally {
        this.busy = false;
      }
    },
