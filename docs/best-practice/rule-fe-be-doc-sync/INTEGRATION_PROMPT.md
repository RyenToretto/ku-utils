...
3. 若有独立合同仓 + 实现仓：添加 `.cursor/skills/doc-backend-sync/SKILL.md`（侦测快照 → 开 `{owner}{seq}-{slug}` 台账 → 逐条提交 → Phase E 完成度表）；可参考 oversea / ai-router 同名 skill；**禁止** `Pnn` / 无 slug；CDP 须 `report.ok=true`。
